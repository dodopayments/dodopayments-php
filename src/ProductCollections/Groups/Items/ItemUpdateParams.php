<?php

declare(strict_types=1);

namespace Dodopayments\ProductCollections\Groups\Items;

use Dodopayments\Core\Attributes\Required;
use Dodopayments\Core\Concerns\SdkModel;
use Dodopayments\Core\Concerns\SdkParams;
use Dodopayments\Core\Contracts\BaseModel;

/**
 * @see Dodopayments\Services\ProductCollections\Groups\ItemsService::update()
 *
 * @phpstan-type ItemUpdateParamsShape = array{
 *   id: string, groupID: string, status: bool
 * }
 */
final class ItemUpdateParams implements BaseModel
{
    /** @use SdkModel<ItemUpdateParamsShape> */
    use SdkModel;
    use SdkParams;

    #[Required]
    public string $id;

    #[Required]
    public string $groupID;

    /**
     * Status of the product in the group.
     */
    #[Required]
    public bool $status;

    /**
     * `new ItemUpdateParams()` is missing required properties by the API.
     *
     * To enforce required parameters use
     * ```
     * ItemUpdateParams::with(id: ..., groupID: ..., status: ...)
     * ```
     *
     * Otherwise ensure the following setters are called
     *
     * ```
     * (new ItemUpdateParams)->withID(...)->withGroupID(...)->withStatus(...)
     * ```
     */
    public function __construct()
    {
        $this->initialize();
    }

    /**
     * Construct an instance from the required parameters.
     *
     * You must use named parameters to construct any parameters with a default value.
     */
    public static function with(string $id, string $groupID, bool $status): self
    {
        $self = new self;

        $self['id'] = $id;
        $self['groupID'] = $groupID;
        $self['status'] = $status;

        return $self;
    }

    public function withID(string $id): self
    {
        $self = clone $this;
        $self['id'] = $id;

        return $self;
    }

    public function withGroupID(string $groupID): self
    {
        $self = clone $this;
        $self['groupID'] = $groupID;

        return $self;
    }

    /**
     * Status of the product in the group.
     */
    public function withStatus(bool $status): self
    {
        $self = clone $this;
        $self['status'] = $status;

        return $self;
    }
}
